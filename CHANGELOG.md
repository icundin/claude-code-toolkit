...
- Dream: `memory.html` normalizes what past runs wrote differently — the night's date is rebuilt from the archive's filename with a fixed month/weekday table (never the machine's locale), `dreamed_at` is forced to `HH:MM`, and `outcome_at`'s five historical shapes (`5 August 2026 15:04`, `13 august 2026 08:20`, `10 August 2026, 08:20`, …) collapse to `YYYY-MM-DD HH:MM`. A `projects` value that is a sentence rather than a list ("no new work sessions since the 17 august dream") now reads as a sentence instead of being cut into chips. `SKILL.md` fixes these shapes for future runs; the script normalizes the ones already archived, since the page is derived from the archives and never edits them.
